...
class PromptId(StrEnum):
    ...
    ANALYZE_CASE_INTELLIGENCE = "analyze_case_intelligence"
...
class PromptSerializationError(ValueError):
    """Raised when runtime context cannot be serialized safely."""
...
_CACHE_KEY_SEGMENT_PATTERN = re.compile(r"[^a-z0-9_-]+")
...
def build_prompt_cache_key(
    ...
    *,
    ...
    parts = [
        ...
        spec.prompt_id.value,
        spec.version,
        spec.model_tier.value,
    ]
    if cache_scope:
        normalized_scope = _CACHE_KEY_SEGMENT_PATTERN.sub(
            "-",
            cache_scope.strip().lower(),
        ).strip("-")
        if normalized_scope:
            parts.append(normalized_scope)
    return ":".join(parts)


def _jsonable(value: Any, path: str = "runtime_context") -> Any:
    if value is None or isinstance(value, (str, int, float, bool)):
        return value
    if hasattr(value, "model_dump"):
        return _jsonable(value.model_dump(mode="json", exclude_none=True), path)
    if is_dataclass(value):
        return _jsonable(asdict(cast(Any, value)), path)
    if isinstance(value, Mapping):
        return {
            str(key): _jsonable(item, f"{path}.{key}") for key, item in value.items()
        }
    if isinstance(value, (list, tuple, set, frozenset)):
        return [_jsonable(item, f"{path}[{index}]") for index, item in enumerate(value)]
    if isinstance(value, Enum):
        return _jsonable(value.value, path)
    raise PromptSerializationError(
        f"Unsupported runtime context value at {path}: {type(value).__name__}"
    )
...
def render_runtime_input(
    ...
    *,
    ...
    if validation_feedback:
        payload["validation_feedback"] = _jsonable(
...
def build_prompt_bundle(
    ...
    *,
    ...
    return PromptBundle(
        ...
        cache_key=build_prompt_cache_key(spec, cache_scope=cache_scope),
...
_register(
    _spec(
        PromptId.ANALYZE_CASE_INTELLIGENCE,
        role=RoleKind.PARTY_STRATEGIST,
        objective=(
            "Analyze the supplied executable training case and produce structured "
            "legal and strategic intelligence."
        ),
        rules=(
            "Identify what each legal element requires and which authored facts support or undermine it.",
            "Map each fact's available proof path through witnesses, documents, statements, and reasonable inferences.",
            "Identify evidence-foundation dependencies, competing factual interpretations, and weak or fragile proof paths.",
            "Identify important strategic opportunities for each side without creating a strategy yet.",
            "Do not validate IDs, schemas, reference integrity, or runtime state.",
            "Do not repair missing evidence, decide the case, or treat a weak fact as invalid.",
            "Report a fact without a proof source as unsupported or currently unprovable; it is a case characteristic, not an execution error.",
        ),
        output_schema="CaseIntelligenceAnalysis",
        tier=ModelTier.CRITICAL_REASONING,
        effort=ReasoningEffort.HIGH,
    )
)
