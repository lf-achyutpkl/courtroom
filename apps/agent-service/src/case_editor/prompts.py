...
def build_process_edit_system_prompt() -> str:
    return (
        ...
        "- Use edit_card when a selected card or a clearly identified existing "
        "card is the target.\n"
        "- Use add_card when the request is asking for a new witness, evidence "
        "item, or disputed fact.\n"
        ...
        "- Align updated_content to the matching CaseFile sub-schema for the "
        "selected card type.\n"
        "- For scoped edits, updated_content must only contain the changed "
        "card payload, not the full case.\n"
        "- Preserve exact field types from the CaseFile schema even when "
        "returning only part of a card.\n"
        "- Do not put descriptive prose into enum fields. Use only literal "
        'enum values such as "prosecution" or "defense".\n'
        '- Evidence.submitted_by must be exactly "prosecution" or "defense".\n'
        '- Witness.called_by must be exactly "prosecution" or "defense".\n'
        "- For edit_card, omit unchanged fields instead of rewriting them with "
        "explanatory text.\n"
        "- Preserve witness information asymmetry. knowledge_scope must remain "
...
def build_process_edit_user_prompt(
    *,
    ...
    return (
        ...
        "Important: updated_content must match the target card schema and enum "
        "fields must use exact literal values."
    )
...
def build_narration_system_prompt() -> str:
    return (
        ...
        "Do not mention internal validation, schemas, or IDs unless the user "
        "asked for them."
    )
