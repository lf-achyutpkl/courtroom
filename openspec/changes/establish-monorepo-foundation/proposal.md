...
The repository has an early Next.js courtroom prototype, but it does not yet have a clear monorepo contract, repo-level agent guidance, or a component architecture that can scale into a web client plus a Python `app/agent-service` workspace. This work is needed now so implementation can proceed against stable repo rules instead of mixing UI, runtime orchestration, and future backend concerns into one code path.

## What Changes

- Define the repository as a polyglot monorepo with a clear separation between the existing `app/web-app` frontend and a planned `app/agent-service` Python/LangGraph workspace.
...
- Affects repository documentation and planning artifacts at the repo root and `app/web-app`.
- Defines future folder ownership for `app/web-app` and `app/agent-service`.
