...
The repository SHALL document `app/agent-service` as the planned workspace for Python and LangGraph runtime code.

#### Scenario: Backend runtime work is introduced
- **WHEN** an agent or developer adds the first backend runtime files
- **THEN** the files are placed under `app/agent-service` instead of being embedded in the web-app workspace
...
- **THEN** the runtime generation logic is assigned to `app/agent-service` and the rendering or playback UI is assigned to `app/web-app`
